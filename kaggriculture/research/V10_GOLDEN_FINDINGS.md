...
- Direct B21 opening run (1,628 games): B18/S13, B19/S14, and B20/S15 each
...
- Exact opening quantities create cliffs: several one-unit changes produced
...
- Current-control validation remains useful but not decisive: B21 passed local
...
- B21 remains the symmetric-opening control: 184/490, score 89,115, margin +835.
- B20/S15 retained 184 wins but fell to 88,811 and +628. It is not a general
  replacement, but is the only lower-transaction opening worth retaining as a
  seat-specific/conditional component.
- B19/S14 retained 184 wins with negative margin; B18/S13 fell to 178 wins.
  B18 is an anti-B21 mirror specialist and must not be promoted as a generalist.
- Exact V7/B21 comparison changed 14 player-tape records: V7 gained both-seat
...
- B21 has 27 player-tape near-loss records within 2,500 coins, representing 23
  unique underlying episodes. These are the primary D/E recovery pool.

## Source C — alternate Arena branch `arena/01a075fa-riemann`

- Its independent opening sweep found the same mirror trap (about 91% for
...
- Its latest old V8-fertilizer validation versus current B21 was 0-200 with
  mean margin -66,343. Preserve only isolated fertilizer mechanics; reject the
  old policy as a competitive base.
- Useful engineering: Windows-safe pytest paths (`pytest.ini`, `conftest.py`,
  `tmp_path`) and a lossless tape-opening patch test.
- Potential acceleration: a Rust/Rayon simulator handoff spec with shared
...
- In the stale heuristic planner, detour radius 1 was strongest directly versus
  old V7, endgame day 24 led field-wide Bradley-Terry, and carrot fraction 0.25
  was a smaller positive lever.
- Hand-count and planted-swap constants frequently produced identical behavior.
  Deduplicate behavior, not merely configuration names.
- These are hypotheses for isolated B21-compatible mutations, never wholesale
  code transfers or current promotion evidence.

## Source E — live Kaggle observations and official mechanics

- Rating uses W/L/T, not coin margin. Small wins are valuable, but mirror-only
  wins can be misleading when the live opponent population is diverse.
- B21 has demonstrated occasional 155k+ games and a promising early climb, yet

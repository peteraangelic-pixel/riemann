"""Kaggriculture V12 agent: V2 + animal buying (TOP12 pattern).

V2 foundation unchanged: wheat/carrot/melon conveyor, reactive crop layout.

V12 addition: BUY COW + SHEEP from d3+ to produce FERTILIZER/MILK/WOOL/EGG.
Fertilizer sold automatically via existing shed-selling logic.
Animals delayed to d3 to not cannibalize initial seed/hand budget.
...
# V12: Animal buying plan (from TOP12 Majkel1337 analysis)
# Animals produce FERTILIZER/MILK/WOOL/EGG which are sold via existing shed logic
_ANIMAL_COST = {"COW": 500, "SHEEP": 500}
_ANIMAL_PLAN = {
    3: {"COW": 1, "SHEEP": 1},   # d3: first wheat harvest money
    6: {"COW": 1, "SHEEP": 1},   # d6: melon + wheat revenue
    9: {"COW": 1, "SHEEP": 1},   # d9: expanding revenue
    12: {"COW": 1, "SHEEP": 1},  # d12: final animals
}
_ANIMAL_RESERVE = 300  # cash to keep after animal purchase
...
class FarmerPlanner:
    ...
    def decide(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        # V12: Animal buying — COW + SHEEP produce FERTILIZER/MILK/WOOL/EGG
        if hour == 0 and day < 29:
            animal_target = _ANIMAL_PLAN.get(day, {})
            if animal_target:
                ...
                for animal, want in animal_target.items():
                    have_a = board_animals.get(animal, 0)
                    to_buy = max(0, want - have_a)
                    for _ in range(min(to_buy, 2)):
                        cost = _ANIMAL_COST.get(animal, 500)
                        if est_money >= cost + _ANIMAL_RESERVE and len(orders) < MAX_MARKET_ORDERS:

"""Kaggriculture v6b price-aware market selector: mixed premium crops and livestock.
...
SELL_PRICE_FLOOR = 1
# V6b: defer non-endgame sales unless the quote clears a product-specific
# premium floor. Farmer/hands schedule remains byte-for-byte V6.
PRICE_AWARE_BASE = {"MILK": 55, "WOOL": 35, "EGG": 28, "FERTILIZER": 20, "MELON": 80, "STRAWBERRY": 55, "CARROT": 35, "WHEAT": 25}
PRICE_AWARE_RATIO = {"MILK": 1.15, "WOOL": 1.10, "EGG": 1.10, "FERTILIZER": 1.10, "MELON": 1.10, "STRAWBERRY": 1.10, "CARROT": 1.15, "WHEAT": 1.0}
...
class FarmerPlanner:
    ...
    def decide(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        for item in sale_items:
            ...
            premium_floor = PRICE_AWARE_BASE.get(item, SELL_PRICE_FLOOR) * PRICE_AWARE_RATIO.get(item, 1.0)
            if price >= (1 if day >= ENDGAME_SELL_DAY else max(SELL_PRICE_FLOOR, premium_floor)):

use kg_sim::{Config, data::Item, rng::PythonRandom};
...
fn cpython_rng_fingerprints_cross_twists_and_64_bit_seed_overflow() {
    ...
    for row in vectors["rng"].as_array().unwrap() {
        ...
        for _ in 0..5000 { hash = hash.wrapping_mul(1_099_511_628_211) ^ u64::from(rng.next_u32()); }
        assert_eq!(hash, row["words_fnv64"].as_u64().unwrap(), "seed={seed} day={day}");
        let mut rng = PythonRandom::for_day(seed, day);
        for pair in row["mixed_calls"].as_array().unwrap() {
            assert_eq!(rng.random().to_bits(), pair[0].as_f64().unwrap().to_bits(), "random(): seed={seed} day={day}");
            assert_eq!(u64::from(rng.choice8()), pair[1].as_u64().unwrap(), "choice(): seed={seed} day={day}");
        }
    }
}
...
fn all_default_price_curves_match_270009_python_quotes() {
    ...
    for row in vectors["prices"].as_array().unwrap() {
        ...
        for inventory in -10000..20001 {
            hash = hash.wrapping_mul(1_099_511_628_211) ^ cfg.curves[item.index()].price(f64::from(inventory)) as u64;
        }

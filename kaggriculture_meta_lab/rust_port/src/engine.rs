...
use crate::{config::Config, data::*, rng::PythonRandom, tape::{Action, Order, OrderKind, UnitAction}};
...
impl Default for Inventory {
    fn default() -> Self {
        Self { amounts: [0; ITEM_COUNT], order: [Item::Wheat; ITEM_COUNT], len: 0 }
    }
}
...
impl Inventory {
    ...
    pub fn take(&mut self, item: Item, n: i64) -> bool {
        let i = item.index();
        if self.amounts[i] < n { return false; }
        self.amounts[i] -= n;
        if self.amounts[i] == 0 {
            let pos = self.order[..self.len].iter().position(|&v| v == item).expect("inventory invariant");
...
impl Unit {
    fn new(pos: [u8; 2]) -> Self { Self { pos, inventory: Inventory::default() } }
}
...
impl Plant {
    fn new(crop: Item, day: i64, turns_per_day: usize) -> Self {
        let cd = crop.crop();
        Self {
            crop, planted_day: day, watered_today: false, consecutive_unwatered: 1,
            yield_units: i32::from(!cd.ongoing),
            max_lifespan_step: if cd.ongoing { -1 } else { (day + cd.max_yield_day + 1) * turns_per_day as i64 },
...
impl Animal {
    fn new(animal: Item, day: i64) -> Self {
        Self {
            animal, placed_day: day, yield_units: 0, consecutive_unfed: 0,
...
#[derive(Clone, Copy, Debug)]
pub(crate) enum Tile { Empty, Locked, Weed, Structure(Structure), Plant(Plant), Animal(Animal) }
...
impl Farm {
    fn new(cfg: &Config, hand_capacity: usize) -> Self {
        ...
        Self {
            ...
            tiles: std::array::from_fn(|i| if quadrant(i % BOARD_SIZE, i / BOARD_SIZE) == 0 { Tile::Empty } else { Tile::Locked }),
            units, unlocked: 1, shed: [0; ITEM_COUNT], shed_total: 0, seeds: [0; CROP_COUNT], hire_fib: [1, 1],
        }
    }

    fn apply_actions(&mut self, action: &Action, cfg: &Config, day: i64, trim_hands: bool) {
        let count = if trim_hands { action.hands.len().min(self.units.len() - 1) } else { action.hands.len() };
        let hands = &action.hands[..count];
        let mut demand = [0_usize; CROP_COUNT];
        for a in std::iter::once(&action.farmer).chain(hands) {
            if let UnitAction::Plant(item) = a { demand[item.index()] += 1; }
        }
        let blocked: [bool; CROP_COUNT] = std::array::from_fn(|i| demand[i] as u64 > self.seeds[i] as u64);
        let allowed = |a: UnitAction| !matches!(a, UnitAction::Plant(item) if blocked[item.index()]);
        if allowed(action.farmer) { self.apply_unit(0, action.farmer, cfg, day); }
        // In raw mode nonexistent hands still contributed to demand above.
        for (i, &a) in hands.iter().take(self.units.len() - 1).enumerate() {
            if allowed(a) { self.apply_unit(i + 1, a, cfg, day); }
        }
    }
    ...
    fn apply_unit(&mut self, idx: usize, action: UnitAction, cfg: &Config, day: i64) {
        ...
        if action == UnitAction::Pass { return; }
        let adjacent = ACCESS.contains(&unit.pos);
        let tile = &mut self.tiles[usize::from(unit.pos[1]) * BOARD_SIZE + usize::from(unit.pos[0])];
        let inv = &mut unit.inventory;
        match action {
            UnitAction::Drop => {
                if adjacent { inv.drop_to_shed(&mut self.shed, &mut self.shed_total, cfg.shed_capacity); }
                return;
            }
            ...
            UnitAction::Place(item, n) => {
                if item.is_animal() && matches!(tile, Tile::Structure(s) if *s == item.animal().structure) {
                    if inv.take(item, 1) { *tile = Tile::Animal(Animal::new(item, day)); }
                    return; // On a matching structure, do not fall through to shed drop.
                }
                if adjacent && n > 0 {
                    let n = n.min(inv.amounts[item.index()]).min((cfg.shed_capacity - self.shed_total).max(0));
        ...
        if matches!(tile, Tile::Locked) { return; }
        match action {
            UnitAction::Plant(crop) if matches!(tile, Tile::Empty) && self.seeds[crop.index()] > 0 => {
                self.seeds[crop.index()] -= 1;
                *tile = Tile::Plant(Plant::new(crop, day, cfg.turns_per_day));
            }
            UnitAction::Water => if let Tile::Plant(p) = tile {
                if p.watered_today { return; }
            ...
            UnitAction::Harvest => match tile {
                Tile::Plant(p) if p.yield_units > 0 => {
                    let cd = p.crop.crop();
                    if day - p.planted_day < cd.first_yield_day { return; }
                    inv.add(p.crop, i64::from(p.yield_units));
                    p.yield_units = 0;
                    if !cd.ongoing { *tile = Tile::Empty; }
                }
            ...
            UnitAction::Fertilize => if let Tile::Plant(p) = tile {
                if inv.take(Item::Fertilizer, 1) { p.fertilized_until_day = p.fertilized_until_day.max(day + 2); }
            },
            UnitAction::Dig if !matches!(tile, Tile::Animal(_)) => *tile = Tile::Empty,
            UnitAction::BuildCoop if matches!(tile, Tile::Empty) => *tile = Tile::Structure(Structure::Coop),
            UnitAction::BuildPasture if matches!(tile, Tile::Empty) => *tile = Tile::Structure(Structure::Pasture),
            UnitAction::Feed => if let Tile::Animal(a) = tile {
                if !a.fed_today && inv.take(Item::Wheat, 1) { a.fed_today = true; }
            },
            UnitAction::CollectFertilizer => if let Tile::Animal(a) = tile {
                if a.fertilizer_available {
                    a.fertilizer_available = false;
                    inv.add(Item::Fertilizer, 1);
                }
            },
            UnitAction::Care => if let Tile::Animal(a) = tile { a.cared_today = true; },
            _ => {}
        }
    }
    ...
    fn hire(&mut self, cfg: &Config) {
        ...
        if (self.money as u128) < cost { return; }
        self.money -= cost as f64;
        self.hire_fib = [self.hire_fib[1], self.hire_fib[0].saturating_add(self.hire_fib[1])];
        let mut occupancy = [0_usize; 4];
        for unit in &self.units {
            if let Some(i) = ACCESS.iter().position(|&p| p == unit.pos) { occupancy[i] += 1; }
        }
        let best = (0..4).min_by_key(|&i| occupancy[i]).expect("four access tiles");
        // A bad caller-provided bound must not silently introduce an allocation.
        assert!(self.units.len() < self.units.capacity(), "insufficient preallocated hand capacity");
        self.units.push(Unit::new(ACCESS[best]));
    }

    fn buy_land(&mut self) {
        if self.unlocked >= 4 || self.money < LAND_PRICES[self.unlocked - 1] { return; }
    ...
    fn commit(&mut self, kind: OrderKind, price: f64, inventory: &mut [f64; PRODUCT_COUNT], cfg: &Config) -> bool {
        match kind {
            OrderKind::Sell(item) => {
                if self.shed[item.index()] <= 0 { return false; }
                self.shed[item.index()] -= 1;
                self.shed_total -= 1;
                self.money += price;
                // Floor-price sales DO NOT increase supply.
                if price > 1.0 { inventory[item.index()] += 1.0; }
            }
            OrderKind::BuyProduct(item) => {
                if self.money < price || self.shed_total >= cfg.shed_capacity { return false; }
                self.money -= price;
                self.shed[item.index()] += 1;
                self.shed_total += 1;
    ...
    fn decay_plants(&mut self, step: i64) {
        for tile in &mut self.tiles {
            let Tile::Plant(p) = tile else { continue; };
            let lifespan = p.max_lifespan_step;
            if lifespan < 0 || step < lifespan || (step - lifespan) % 2 != 0 { continue; }
            p.yield_units -= 1;
            if p.yield_units <= 0 { *tile = Tile::Weed; }
        }
    }
    ...
    fn refresh_plants(&mut self, day: i64, turns_per_day: usize) {
        ...
        for tile in &mut self.tiles {
            ...
            p.consecutive_unwatered = if watered { 0 } else { p.consecutive_unwatered + 1 };
            p.watered_today = false;
            if p.consecutive_unwatered >= 2 { *tile = Tile::Weed; continue; }
            let cd = p.crop.crop();
            if !cd.ongoing { continue; }
            let since_first = next_day - p.planted_day - cd.first_yield_day;
            if since_first < 0 || since_first % cd.interval != 0 { continue; }
            let count = since_first / cd.interval + 1;
            if count > i64::from(cd.max_yield) { continue; }
            let fertilized = watered && p.fertilized_until_day >= day;
            p.yield_units = cd.max_yield.min(p.yield_units + if fertilized { 2 } else { 1 });
            if count == i64::from(cd.max_yield) { p.max_lifespan_step = (next_day + 1) * turns_per_day as i64; }
        }
    }

    fn refresh_animals(&mut self, day: i64) {
        for tile in &mut self.tiles {
            let Tile::Animal(a) = tile else { continue; };
            let ad = a.animal.animal();
            a.consecutive_unfed = if a.fed_today { 0 } else { a.consecutive_unfed + 1 };
            if a.consecutive_unfed >= 2 { *tile = Tile::Structure(ad.structure); continue; }
    ...
    fn end_day(&mut self, cfg: &Config, day: i64, rng: &mut PythonRandom) {
        ...
        for tile in &mut self.tiles {
            ...
            if matches!(tile, Tile::Empty) && rng.random() < cfg.weed_chance { *tile = Tile::Weed; }
        }
        for unit in &mut self.units {
            unit.inventory.drop_to_shed(&mut self.shed, &mut self.shed_total, cfg.shed_capacity);
        }
...
impl<'a> Game<'a> {
    ...
    pub fn new(config: &'a Config, seed: i64, hand_capacities: [usize; 2]) -> Self {
        Self {
            ...
            shops: [0; SHOP_COUNT], shop_len: 0, market_loop_aborts: 0,
        }
    }

    pub fn turn(&self) -> usize { self.turn }
    pub fn rewards(&self) -> [f64; 2] { [self.farms[0].money, self.farms[1].money] }
    pub fn market_loop_aborts(&self) -> usize { self.market_loop_aborts }
    ...
    pub fn step(&mut self, actions: [&Action; 2], trim_hands: bool) {
        ...
        self.town_consume();
        for farm in &mut self.farms { farm.decay_plants(self.turn as i64); }
        if (self.turn + 1) % self.config.turns_per_day == 0 {
            ...
            for farm in &mut self.farms { farm.end_day(self.config, day, &mut rng); }
            if (day + 1) % self.config.shop_unlock_interval as i64 == 0 && self.shop_len < SHOP_COUNT {
    ...
    fn process_market(&mut self, actions: [&Action; 2]) {
        let count = actions[0].market.len().max(actions[1].market.len()).min(self.config.max_market_orders);
        for index in 0..count {
            let mut orders: [Order; 2] = std::array::from_fn(|i| actions[i].market.get(index).copied().unwrap_or_default());
            for (i, order) in orders.iter_mut().enumerate() {
                match order.kind {
                    OrderKind::Hire => { self.farms[i].hire(self.config); order.kind = OrderKind::Noop; }
                    OrderKind::BuyLand => { self.farms[i].buy_land(); order.kind = OrderKind::Noop; }
                    _ => {}
                }
            }
            ...
            loop {
                iterations += 1;
                if iterations >= 100_000 { self.market_loop_aborts += 1; break; }
                // Quote BOTH against the same inventory BEFORE either commits.
                let quotes: [Option<f64>; 2] = std::array::from_fn(|i| {
                    if orders[i].remaining == 0 { return None; }
                    match orders[i].kind {
                        OrderKind::Sell(item) => Some(self.config.curves[item.index()].price(self.market_inventory[item.index()])),
                        OrderKind::BuyProduct(item) => Some(self.config.curves[item.index()].price(self.market_inventory[item.index()] - 1.0)),
                ...
                for (i, price) in quotes.into_iter().enumerate() {
                    if let Some(price) = price {
                        if self.farms[i].commit(orders[i].kind, price, &mut self.market_inventory, self.config) {
                            orders[i].remaining -= 1;
                            committed_any = true;
                        } else { orders[i].kind = OrderKind::Noop; }
                    }
                }
    ...
    fn town_consume(&mut self) {
        if self.turn % self.config.shop_sell_interval == 0 {
            for &id in &self.shops[..self.shop_len] {
                ...
                for &item in products { self.market_inventory[item.index()] -= multiplier; }
            }
        }
        if self.turn % self.config.center_sell_interval == 0 {
            for inventory in &mut self.market_inventory[..PRODUCT_COUNT - 1] { *inventory -= 1.0; }
        }
    }
}
...
mod tests {
    ...
    fn shared_market_quotes_are_simultaneous() {
        ...
        let tape = Tape::from_json(&json!([[{"market": [["BUY_PRODUCT", "WHEAT", 4]]}], [{}]])).unwrap();
        ...
        for inv in [9999.0, 9997.0, 9995.0, 9993.0] { expected -= cfg.curves[0].price(inv); }
    ...
    fn phantom_hands_block_planting_unless_explicitly_trimmed() {
        let cfg = Config::default();
        let tape = Tape::from_json(&json!([[{"farmer": ["PLANT", "WHEAT"], "hands": [["PLANT", "WHEAT"]]}], [{}]])).unwrap();
    ...
    fn floor_sales_do_not_inflate_supply() {
        ...
        assert!(game.farms[0].commit(OrderKind::Sell(Item::Melon), 1.0, &mut game.market_inventory, &cfg));

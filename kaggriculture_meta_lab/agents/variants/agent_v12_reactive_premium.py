"""V12 reactive premium planner: state-scored routing plus competitive production scale.

This generation keeps the proven V7 production kernel and adds a cheap shared
candidate scorer: urgent watering, harvest, planting, fertilizing, weeds, and
animal work compete on live state value minus travel cost. Same-turn target
claims prevent workers from marching toward one tile. The evening water bonus
starts at hour 17, based on the independently audited FarmOS v12 idea, while
fertilizing is gated behind watering to avoid v13's measured detour failure.

Evolution remains bounded to explicit numeric genes; broad population search
must wait until this planner clears small closed-loop screens.

Historical base: V10 research kernel for bounded evolution from the V7 control.
...
MELON_CELLS = [(x, 0) for x in range(5)] + [(x, 1) for x in range(4)] + [(x, 2) for x in range(3)]
...
# Fast utility scale for live unit routing. A complete decision is O(U*C): at
# most 15 units score at most 100 board cells, negligible beside simulation.
MOVE_SCORE_COST = 12
SCORE_URGENT_WATER = 1300
SCORE_HARVEST = 900
SCORE_WATER = 620
SCORE_PLANT = 500
SCORE_FERTILIZE = 300
...
def _select_target(
    ...
    value_bonus: dict[tuple[int, int], int] | None = None,
) -> tuple[int, int] | None:
    """Choose and claim the highest-value safe target.

    ``groups`` carries task labels and candidate cells. The function is pure
    except for claiming the selected destination, making simultaneous worker
    allocation deterministic and directly unit-testable.
    """
    base = {
        "urgent_water": SCORE_URGENT_WATER,
        "harvest": SCORE_HARVEST,
        "liquidate": 2600,
        "water": SCORE_WATER,
        "plant": SCORE_PLANT,
        "fertilize": SCORE_FERTILIZE,
        "dig": SCORE_DIG,
    }
    best: tuple[int, int, int, int, int] | None = None
    best_cell: tuple[int, int] | None = None
    bonuses = value_bonus or {}
    for task, cells in groups:
        task_score = base[task]
        if task in ("urgent_water", "water") and hour >= EVENING_WATER_HOUR:
            task_score += EVENING_WATER_BONUS
        for x, y in cells:
            cell = (x, y)
            if cell in claimed:
                continue
            distance = abs(x - fx) + abs(y - fy)
            score = task_score + int(bonuses.get(cell, 0)) - MOVE_SCORE_COST * distance
            key = (score, -distance, -y, -x, -len(claimed))
...
class FarmerPlanner:
    ...
    def decide(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        # Shared per-turn reservation prevents farmer/hands from selecting the
        # same destination. Harvest bonus is intentionally capped: urgency and
        # travel still dominate speculative price chasing.
        claimed: set[tuple[int, int]] = set()
        target_value: dict[tuple[int, int], int] = {}
        for x, y in mature:
            tile = tiles[y][x]
            target_value[(x, y)] = min(
                300,
                int(prices.get(tile.get("crop"), 0) or 0)
                * max(1, int(tile.get("yield_units", 0) or 0)) // 4,
            )
        ...
        def _plant_ok(crop: str) -> bool:
            ...
            if plants_assigned[crop] >= int(seeds.get(crop, 0)):
                return False
            if hour > 22 or day > PLANT_STOP_DAY:
                return False
            return True
        ...
        def _standing_op(
            ...
            if isinstance(tile, dict) and tile.get("kind") == "PLANT" and tile.get("crop") in SUPPORTED:
                ...
                if (
                    (fx, fy) in fertilizable
                    and tile.get("watered_today")
                    and int(inventory.get("FERTILIZER", 0)) > 0
                ):
                    return ["FERTILIZE"]
                if age >= hage and yld > 0:
                    if age == hage and not tile.get("watered_today") and day < ENDGAME_HARVEST_DAY:
        ...
        def _farm_op(
            ...
            watered_fertilize = [c for c in z_fertilize if tiles[c[1]][c[0]].get("watered_today")]
            if watered_fertilize and int(inventory.get("FERTILIZER", 0)) <= 0 and int(shed.get("FERTILIZER", 0)) > 0:
                if (fx, fy) == (4, 4):
                    return ["PICKUP", "FERTILIZER", min(FERTILIZER_RESERVE, int(shed["FERTILIZER"]))]
                return [_walk(fx, fy, 4, 4)]
            safe_fertilize = [
                c for c in z_fertilize if tiles[c[1]][c[0]].get("watered_today")
            ] if int(inventory.get("FERTILIZER", 0)) > 0 else []
            target = _select_target(
                [
                    ("urgent_water", z_urgent),
                    ("liquidate" if day >= ENDGAME_HARVEST_DAY else "harvest", z_mature),
                    ("water", z_wet),
                    ("plant", z_plant),
                    ("fertilize", safe_fertilize),
                    ("dig", [c for c in weeds if c in zone_set]),
                ],
                fx, fy, hour, claimed, target_value,
            )
            if target is not None:
        ...
        def _hand_op(
            ...
            if target is None and IDLE_WORK_STEAL:
                target = _select_target(
                    [("liquidate" if day >= ENDGAME_HARVEST_DAY else "urgent_water", mature if day >= ENDGAME_HARVEST_DAY else urgent),
                     ("water", [] if day >= ENDGAME_HARVEST_DAY else unwatered),
                     ("harvest", [] if day >= ENDGAME_HARVEST_DAY else mature)],
                    fx, fy, hour, claimed, target_value,
                )
            if target is not None:
                mv = _walk(fx, fy, *target)
...
EVOLUTION_BOUNDS = {
    ...
    "MOVE_SCORE_COST": (4, 30),
    "SCORE_HARVEST": (500, 1500),
    "SCORE_WATER": (300, 1100),
    "SCORE_PLANT": (200, 900),
    "SCORE_FERTILIZE": (100, 700),
    "EVENING_WATER_HOUR": (14, 21),
    "EVENING_WATER_BONUS": (100, 900),
}

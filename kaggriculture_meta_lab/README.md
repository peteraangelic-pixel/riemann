...
```powershell
python scripts\sweep.py --config sweeps\v10_b21_opening.json --workers 16
```

That default projects about 1,250 full games. `sweep.py` now prints the projected
...
(gitignored). The untouched base agent is always added as `BASE_*` control.
Results land in committable `results/sweep-<name>-<timestamp>.md` (+`.json`).

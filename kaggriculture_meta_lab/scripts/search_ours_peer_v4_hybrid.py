...
def fit(s):
 direct=min(s['ours']['score_rate'],s['peer']['score_rate']);dm=min(s['ours']['mean_margin'],s['peer']['mean_margin']);ext=[v for k,v in s.items() if k not in ('ours','peer')]
 return (direct,dm,min(x['score_rate'] for x in ext),statistics.mean(x['score_rate'] for x in ext),statistics.mean(x['mean_margin'] for x in ext))
def render(a,note):
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='ours-peer-hybrid-') as raw:
  ...
  for gen in range(a.generations):
   seeds=range(95000+gen*101,95008+gen*101);stats,j=evaluate(pop,src,controls,seeds,a.binary,a.threads,td,f'g{gen}');total+=j;rank=sorted(range(len(pop)),key=lambda i:fit(stats[i]),reverse=True);hist.append({'generation':gen,'best_genome':list(pop[rank[0]]),'best':stats[rank[0]],'fitness':list(fit(stats[rank[0]]))});elite=[pop[i] for i in rank[:48]]

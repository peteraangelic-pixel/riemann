...
def stat(rs):
 if not rs:return {'games':0,'wins':0,'losses':0,'mean_reward':None,'mean_margin':None}
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--corpus',type=Path,required=True);p.add_argument('--safe',type=Path,required=True);p.add_argument('--router',type=Path,required=True);p.add_argument('--workers',type=int,default=4);p.add_argument('--seed',type=int,default=195000);p.add_argument('--target-ranks',default='8');p.add_argument('--negative-ranks',default='7,11,12');p.add_argument('--output',type=Path,required=True);a=p.parse_args()
 target={int(x) for x in a.target_ranks.split(',') if x};negative={int(x) for x in a.negative_ranks.split(',') if x}
 temp,meta=build_top30_jobs(a.corpus.resolve(),a.safe.resolve(),12);records=[]
 for i in range(0,len(temp),2):
  if meta[i]['rank'] in target|negative:records.append((temp[i][2],meta[i],'target' if meta[i]['rank'] in target else 'negative'))
 jobs=[];labels=[]
 for name,path in [('safe',a.safe),('router',a.router)]:
  for spec,m,kind in records:
   for seat in (0,1):jobs.append((a.seed,str(path.resolve()),spec,seat,720,kind));labels.append((name,m,kind))

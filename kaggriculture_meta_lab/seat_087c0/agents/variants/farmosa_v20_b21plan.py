...
HERD = {"COW": [(0, 2), (3, 4), (6, 6), (9, 8)],
        "SHEEP": [(1, 2), (8, 4), (10, 8), (12, 9)],
        "GOOSE": [(12, 3)]}
BUY_WINDOW = {"COW": 13, "SHEEP": 17, "GOOSE": 15}
STRAW_CAP = 34
CARROT_CAP = 36
MELON_TARGET = 12
...
def _decide(obs, mem):
    ...
    if "NE" in unlocked and 8 <= day <= 15 and straw_total < STRAW_CAP \
            and len(orders) < 9:
    ...
    for kind, schedule in HERD.items():
        ...
        while want > 1 and money - est_spend - cost < floor():
            want -= 1
            cost = ANIMALS[kind]["cost"] * want
        if want >= 1 and room >= want and len(orders) < 9:
            if day <= 3 and money - est_spend - cost < floor() + 250:
                continue  # keep cash for d1-3 hiring/feed only
            if afford(cost):
    ...
    # ============================ WORKERS =================================
    wheat_goal = min(45, max(0, feed_need + 12)) if day >= 8 else 0
    ...
    def _crop_here(x, y):
        ...
        if (x, y) in MELON_CELLS:
            if day <= 2 and plants.get("MELON", 0) < MELON_TARGET:
                return "MELON"
            if 10 <= day <= 15 and plants.get("STRAWBERRY", 0) < STRAW_CAP \
                    and "NE" in unlocked:
                return "STRAWBERRY"   # melon rows join the strawberry bed
            if day >= 23 and plants.get("CARROT", 0) < CARROT_CAP:
                return "CARROT"
            if 13 <= day <= 23 and plants.get("WHEAT", 0) < wheat_goal:
                return "WHEAT"
            return None
        if (x, y) in CROP_NE:
            ...
            if day >= 23 and plants.get("CARROT", 0) < CARROT_CAP:
                return "CARROT"
            if 8 <= day <= 23 and plants.get("WHEAT", 0) < wheat_goal:
                return "WHEAT"
            return None
    ...
    def _needs_water(t):
        ...
        if not t.get("watered_today"):
            ...
            if t.get("consecutive_unwatered", 0) >= 1:
                return True       # watered yesterday: must not skip two days
            if not cd["ongoing"]:
                return age <= cd["max_day"] + 1
            # ongoing crops accumulate yield even unwatered; they tolerate a
            # dry day as long as we never miss two in a row (handled above)
            return False
        return False
    ...
    def worker_step_hand(hid, pos, inv):
        ...
        if hour >= 15 and not carrying_animal:

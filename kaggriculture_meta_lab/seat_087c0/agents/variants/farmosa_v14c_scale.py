...
SELL_CAP = {"WHEAT": 16, "CARROT": 10, "TOMATO": 2, "STRAWBERRY": 8, "MELON": 12,
            "EGG": 6, "MILK": 8, "WOOL": 5, "FERTILIZER": 10}
SELL_FLOOR = {"WHEAT": 5, "CARROT": 5, "TOMATO": 5, "STRAWBERRY": 20, "MELON": 60,
              "EGG": 8, "MILK": 20, "WOOL": 15, "FERTILIZER": 60}

HERD = {"COW": [(0, 2), (3, 4), (6, 6), (9, 8)],
        "SHEEP": [(1, 2), (4, 3), (6, 4), (9, 5)],
        "GOOSE": [(12, 3)]}
BUY_WINDOW = {"COW": 13, "SHEEP": 16, "GOOSE": 15}
STRAW_CAP = 20
CARROT_CAP = 36
MELON_TARGET = 12
...
def _decide(obs, mem):
    ...
    if day < 30:
        wheat_keep = 0 if day >= 28 else max(feed_need + 12, 16)
        fert_keep = 0 if day >= 28 or day < 10 else 2
        ...
        for item in ("FERTILIZER", "EGG", "MILK", "WOOL", "MELON",
            ...
            if item == "WHEAT":
                ...
                keep = 0 if day >= 28 else fert_keep
                if stock <= keep:
                    continue
            else:
                keep = 0
            ...
            if day < 28 and price < SELL_FLOOR[item]:
                continue
    ...
    for kind, schedule in HERD.items():
        ...
        if want >= 1 and room >= want and len(orders) < 9:
            if day <= 9 and money - est_spend - cost < floor() + 300:
                continue  # keep liquidity for next day's hiring
            if afford(cost):
    ...
    # ============================ WORKERS =================================
    wheat_goal = min(40, max(20, feed_need + 12)) if day >= 10 else 0
